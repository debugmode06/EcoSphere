...
function ResolveModal({ open, onClose, issue, onResolved }) {
  ...
  const handleSubmit = async (e) => {
    ...
    setError('');
    try {
      const res = await resolveIssue(issue._id, { resolutionNotes: notes });
...
// ─── Issue Row ────────────────────────────────────────────────────────────────
function IssueRow({ issue, canResolve, onResolve }) {
  return (
    <tr>
            ...
            {loading ? (
              ...
              <tr>
                <td colSpan={6} className="py-16 text-center">
                  ...
                  {canEdit && statusFilter === 'ALL' && severityFilter === 'All' && !search && (
                    <button onClick={() => setShowCreate(true)} className="btn-primary mt-3 text-sm">
                      <Plus className="w-4 h-4 inline mr-1" /> Log First Issue
                    </button>
                  )}
                </td>
              </tr>
